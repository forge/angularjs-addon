...
<#if (property.hidden!"false") == "false">
    <div class="form-group" ng-class="{'has-error': ${formProperty}.$invalid}">
        ...
        <select id="${property.name}" name="${property.name}" class="form-control" ng-model="${property.identifier}Selection" ng-options="${collectionVar}.text for ${collectionVar} in ${collection}" <#if (property.required!) == "true">required</#if> >
